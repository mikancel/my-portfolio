...
async function compressImage(file: File): Promise<CompressResult | null> {
  if (file.type === "image/gif") {
    return { blob: file, contentType: "image/gif", filename: file.name };
  }
...
export default function PostEditor({ postId: initialPostId }: { postId: string | null }) {
  const [postId, setPostId] = useState<string | number | null>(initialPostId || null);
  ...
  // アプリ内リンク（「← ブログ管理」等）での離脱も、未保存なら確認を挟む。
  // beforeunload はタブ閉じ/リロードしか拾わないため、クリックをcapture段階で横取りする
  useEffect(() => {
  ...
  useEffect(() => {
    if (!isEdit) return;
    fetch(`/api/blog/${initialPostId}?all=1`)
      .then(r => r.json())
  ...
  const ensurePostId = useCallback(async (): Promise<string | number | null> => {
  ...
  const uploadFiles = useCallback(async (files: File[], isThumbnail = false) => {
    ...
    for (const file of mediaFiles) {
      ...
      const data: { error?: string; url?: string; publicUrl?: string } = await res.json();
      if (data.error || !data.url || !data.publicUrl) {
  ...
  const handleSave = async (pub: boolean = published) => {
    if (saving) return;
    if (!title.trim()) {
      setMessage({ type: "error", text: "タイトルを入力してください" });
      return;
