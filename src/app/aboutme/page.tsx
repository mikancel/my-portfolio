...
type ScriptLine = { type: "prompt" | "out"; text: string };
type TermLine = ScriptLine | { type: "blank" };
type WhoamiKey = "name" | "role" | "location";
...
function CmdWindow({
  ...
  onWhoami: (what: WhoamiKey) => void;
  onClosed?: () => void;
}) {
  const [lines, setLines] = useState<TermLine[]>([]);
  const [step, setStep] = useState(0);
  const [typing, setTyping] = useState("");
  const [phase, setPhase] = useState<"booting" | "typing" | "waiting">("booting");
...
export default function AboutMe() {
  ...
  const handleWhoami = useCallback((what: WhoamiKey) => {
