...
function useScrollHeader() {
  ...
  const applyTranslate = useCallback((val: number) => {
  ...
  const setTocOpenRef = useCallback((val: boolean) => {
  ...
  const handleTocClick = useCallback<TocClickHandler>((e, headingId) => {
    if (headingId === null) {
      applyTranslate(0);
      return;
    }
...
export default function PostClient({
  ...
  useEffect(() => {
    ...
    container.querySelectorAll("pre").forEach((pre) => {
      ...
      wrapBtn.onclick = () => {
        wrapped = !wrapped;
        if (code) code.style.whiteSpace = wrapped ? "pre-wrap" : "pre";
